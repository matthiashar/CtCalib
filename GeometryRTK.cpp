...
GeometryRTK::GeometryRTK(double SDD, double SRD, cv::Size2i imageSize, double pixelSize)
{
	...
	m_parameter = {
	GeometryParameter("SRD", SRD, AdjustEnumOptions::ADJUST_PARAM),
		GeometryParameter("SDD", SDD, AdjustEnumOptions::CONST_PARAM),
		...
		GeometryParameter("SourceOffsetX", 0, AdjustEnumOptions::ADJUST_PARAM),
		GeometryParameter("SourceOffsetY", 0, AdjustEnumOptions::ADJUST_PARAM),
		GeometryParameter("GantryAngleStep", 0, AdjustEnumOptions::ADJUST_PARAM)
	};
}


Eigen::Matrix<double, 3, 4> GeometryRTK::getProjectionMatrix(double gantry_angle)
{
	...
	Eigen::Matrix<double, 3, 4> projMatrix = projectionMatrix<double>(
		...
		&_plist["SourceOffsetX"]->m_value,
		&_plist["SourceOffsetY"]->m_value,
		&_plist["SDD"]->m_value,
		&_plist["SRD"]->m_value);
	return projMatrix;
}

ceres::ResidualBlockId GeometryRTK::addResidualBlock(
	...
	double& y)
{
	...
	ceres::CostFunction* cost_function = rtkReprojectionError::Create(x, y);
